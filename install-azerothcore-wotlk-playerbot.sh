...
function install(){
	...
	if [[ -d ${AC_CODE_DIR} ]]; then
		...
		git clone https://github.com/mod-playerbots/azerothcore-wotlk.git --branch=Playerbot ${AC_CODE_DIR}
		...
		sudo mysql -e "GRANT ALL PRIVILEGES ON acore_playerbots.* TO '${DB_USER}'@'localhost';"
		...
		echo "################################################"
		cat <<-EOF | sudo tee /etc/systemd/system/acbot-authserver.service > /dev/null
			[Unit]
			Description=AzerothCore Playerbot Authserver
		...
		cat <<-EOF | sudo tee /etc/systemd/system/acbot-worldserver.service > /dev/null
			[Unit]
			Description=AzerothCore Player Worldserver
			After=network.target
			StartLimitIntervalSec=0
